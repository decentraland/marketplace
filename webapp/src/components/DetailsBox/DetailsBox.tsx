...
const isActiveRental = (status: RentalStatus) => status === RentalStatus.OPEN || status === RentalStatus.EXECUTED

export const DetailsBox = (props: Props) => {
  const { asset, rental, className } = props
  const order = useGetCurrentOrder()

  const owner = rental?.lessor && isActiveRental(rental.status) ? rental.lessor : isNFT(asset) ? asset.owner : asset.creator
