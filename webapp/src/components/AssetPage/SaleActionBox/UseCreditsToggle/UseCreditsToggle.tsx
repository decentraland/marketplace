...
const UseCreditsToggle = ({ assetPrice, credits, isOwner, useCredits, onUseCredits }: Props) => {
  ...
  const creditsInEth = useMemo(() => {
    ...
    const maxBetweenAssetPriceAndCreditsToUse = assetPrice ? Math.min(Number(assetPriceInEth), Number(inEth)) : inEth

    return maxBetweenAssetPriceAndCreditsToUse
        ...
        <span className={styles.creditsAmount}>
          <Button
            ...
            onClick={() => {
              ...
              window.open('https://docs.decentraland.org/decentraland/credits/', '_blank')
            }}
          >
            {t('global.learn_more')}
          </Button>
    ...
    <div className={`${styles.creditsContainer} ${useCredits ? styles.creditsContainerActive : ''}`}>
      ...
      <span className={styles.creditsAmount}>
        <Popup
          ...
          position="top center"
          trigger={<span className={styles.creditsNumber}>{creditsInEth}</span>}
          on="hover"
        />
      </span>
    </div>
