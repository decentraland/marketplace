...
const BuyNFTModal = (props: Props) => {
  ...
  let subtitle = null
  if (!order) {
    subtitle = <T id={'buy_with_mana_page.not_for_sale'} values={{ name }} />
  } else if (
    !fingerprint &&
    nft.category === NFTCategory.ESTATE &&
    !isFingerprintLoading
  ) {
    subtitle = <T id={'buy_with_mana_page.no_fingerprint'} />
  } else if (isOwner) {
    subtitle = <T id={'buy_with_mana_page.is_owner'} values={{ name }} />
  } else if (hasInsufficientMANA) {
    subtitle = (
      <T
        id={'buy_with_mana_page.not_enough_mana'}
        values={{
          name,
      ...
      <div className="buttons">
        ...
        {!hasLowPrice ? (
          ...
          >
            ...
            {t('buy_with_mana_page.buy_with_mana')}
          </ChainButton>
        ) : null}
      </div>
      {authorization ? (
