import { BuildEntityWithoutFilesOptions, CatalystClient, DeploymentPreparationData } from 'dcl-catalyst-client'
...
export const fetchStoreEntity = async (client: CatalystClient, address: string): Promise<Entity | null> => {
...
export const deployStoreEntity = async (client: CatalystClient, identity: AuthIdentity, store: Store) => {
  const { owner } = store
