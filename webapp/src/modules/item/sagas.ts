...
import { call, cancel, cancelled, delay, fork, race, select, take } from 'redux-saga/effects'
...
export function* itemSaga(getIdentity: () => AuthIdentity | undefined) {
  ...
  function* handleBuyItemCrossChainSuccess(action: BuyItemCrossChainSuccessAction) {
    ...
    if (route.requestId && route.route.params.fromChain !== route.route.params.toChain) {
      ...
      while (!status || !status?.toChain?.transactionId) {
        ...
        try {
          status = yield call([crossChainProvider, 'getStatus'], route.requestId, txHash)
        } catch (error) {
      ...
      if (status?.toChain?.transactionId) {
        yield put(trackCrossChainTx(destinationChain, status?.toChain?.transactionId))
        yield put(
          push(
            locations.success({
              txHash,
              ...
              contractAddress: order ? order.contractAddress : item.contractAddress,
              isCrossChain: ('route' in action.payload).toString()
            })
          )
        )
        yield put(showToast(getCrossChainTransactionSuccessToast(AxelarProvider.getTxLink(txHash))))
      }
    }
  }
