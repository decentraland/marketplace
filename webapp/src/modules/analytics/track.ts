...
track<AcceptBidTransactionSubmittedAction>(ACCEPT_BID_TRANSACTION_SUBMITTED, 'Accept bid', ({ payload }) => ({
...
track<CancelBidSuccessAction>(CANCEL_BID_SUCCESS, 'Cancel bid', ({ payload }) => ({
  tokenId: payload.bid.tokenId,
  bidId: payload.bid.id,
  bidder: payload.bid.bidder
}))
...
track<FetchNFTsSuccessAction>(FETCH_NFTS_SUCCESS, 'Fetch NFTs', ({ payload }) => ({
...
track<FetchItemsSuccessAction>(FETCH_ITEMS_SUCCESS, 'Fetch Items', ({ payload }) => ({
...
track<BuyItemWithCardSuccessAction>(BUY_ITEM_WITH_CARD_SUCCESS, 'Buy Item With Card', ({ payload }) => ({
  itemId: payload.item.itemId,
  contractAddress: payload.item.contractAddress,
  rarity: payload.item.rarity,
  network: payload.item.network,
...
track<SetIsTryingOnAction>(SET_IS_TRYING_ON, 'Toggle Preview Mode', ({ payload }) => ({
  mode: payload.value ? 'avatar' : 'wearable'
}))

track<UpsertRentalSuccessAction>(UPSERT_RENTAL_SUCCESS, 'Upsert Land Rental', ({ payload: { nft, operationType, rental } }) => ({
  nftId: nft.id,
...
track<ClaimAssetSuccessAction>(CLAIM_ASSET_SUCCESS, 'Claim Land Rental', ({ payload: { nft, rental } }) => ({
  nftId: nft.id,
  rentalId: rental.id
}))

track<AcceptRentalListingSuccessAction>(ACCEPT_RENTAL_LISTING_SUCCESS, 'Rent Land', ({ payload: { periodIndexChosen, rental, nft } }) => ({
  nftId: rental.nftId,
  assetType: isParcel(nft) ? NFTCategory.PARCEL : NFTCategory.ESTATE,
  rentalId: rental.id,
