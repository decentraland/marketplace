...
describe.each([
  ...
  [ListsBrowseSortBy.RECENTLY_UPDATED, ListsSortBy.UPDATED_AT, SortDirection.DESC]
])('when converting the %s sortBy into an API sortBy', (sortBy, expectedAPISortby, expectedAPISortDirection) => {
...
describe('when converting a wrong sortBy value into an API sortBy', () => {
  it('should throw an error signaling that the sortBy value is wrong', () => {
    expect(() => convertListsBrowseSortByIntoApiSortBy('wrong' as ListsBrowseSortBy)).toThrowError('Wrong sortBy value')
  })
})
