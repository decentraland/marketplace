...
export const getRentalById = (state: RootState, id: string): RentalListing | null => getData(state)[id] ?? null
export const isClaimingAsset = (state: RootState) => isLoadingType(getLoading(state), CLAIM_ASSET_REQUEST)
export const isRemovingRental = (state: RootState) => isLoadingType(getLoading(state), REMOVE_RENTAL_REQUEST)
export const isAcceptingRental = (state: RootState) => isLoadingType(getLoading(state), ACCEPT_RENTAL_LISTING_REQUEST)
export const isSubmittingTransaction = (state: RootState) => getState(state).isSubmittingTransaction
